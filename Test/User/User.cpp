...
{
	setUsername("Admin");
	std::cout << "Stworzono admina." << std::endl;
	setPassword("");
	setSID("a-1");
}
void User::createGuest() {
	setUsername("Guest");
	std::cout << "Stworzono goscia." << std::endl;
	...
	if (currentLoggedGotAdminPermissions() == true) {
			...
			if (user->getSID()[0] == 'a') {
				...
				for (it = User::adminGroup.begin(); it != User::adminGroup.end(); it++) {
					if ((*it)->getSID() == user->getSID()) {
						std::cout << "Usunieto z grupy adminow "  << (*it)->getUsername() << std::endl;
			...
			if (user->getSID()[0] == 's') {
				...
				for (it = User::standardUserGroup.begin(); it != User::standardUserGroup.end(); it++) {
					if ((*it)->getSID() == user->getSID()) {
						std::cout << "Usunieto z grupy standardowych uzytkownikow  " << (*it)->getUsername() << std::endl;
			...
			for (it = User::userList.begin(); it != User::userList.end(); it++) {
				if ((*it)->getSID() == user->getSID()) {
					std::cout << "Usunieto z grupy uzytkownikow " << (*it)->getUsername() << std::endl;
